...
| 3770134628 | Critter Overlay                        | guybrush.CritterOverlay |
| 3778736902 | GRAVNet AI and Infrastructure          | germany7496.MEEPNetAI   |
...
| 3778736902 | GRAVNet AI and Infrastructure | 标准键由 `localizations/GRAVNetAI/zh.json` 的 `translations` 段经 `Strings.Get` 补丁注入；约 27 处内联硬编码 UI 文本由 `MeepNetPatches` 拦截 `LocText.set_text` + `TMP_Text.set_text` 应用（exact / replace） |
